...
def fig4_disease_site(trials, arms=None):
    """Grouped box plot by disease site, ordered by median TT.
    Uses arm-level intervention data when available for consistency
    with manuscript statistics (N=790 intervention arms, median=19)."""
    fig, ax = plt.subplots(figsize=(12, 6))

    if arms is not None:
        data = arms[(arms["intervention_type"] == "intervention")].dropna(
            subset=["12_months", "disease_site"])
        tt_col = "12_months"
    else:
        data = trials.dropna(subset=["intervention_12_months", "disease_site"])
        tt_col = "intervention_12_months"

    # Order by median descending
    order = data.groupby("disease_site")[tt_col].median().sort_values(ascending=False).index

    sns.boxplot(data=data, x="disease_site", y=tt_col,
...
def main():
    ...
    fig4_disease_site(trials, arms=arms)
